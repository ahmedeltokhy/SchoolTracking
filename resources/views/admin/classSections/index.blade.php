...
@can('class_section_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.class-sections.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.classSection.title_singular') }}
            </a>
        </div>
    </div>
@endcan
                ...
                <tbody>
                    @foreach($classSections as $key => $classSection)
                        <tr data-entry-id="{{ $classSection->id }}">
                            ...
                            <td>
                                @can('class_section_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.class-sections.show', $classSection->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('class_section_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.class-sections.edit', $classSection->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('class_section_delete')
                                    <form action="{{ route('admin.class-sections.destroy', $classSection->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('class_section_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.class-sections.massDestroy') }}",
