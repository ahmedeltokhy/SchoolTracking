...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.checkStation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.check-stations.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bus_station') ? 'is-invalid' : '' }}" name="bus_station_id" id="bus_station_id" required>
                    @foreach($bus_stations as $id => $entry)
                        <option value="{{ $id }}" {{ old('bus_station_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
