                ...
                <tbody>
                    @foreach($stations as $key => $station)
                        <tr data-entry-id="{{ $station->id }}">
                            ...
                            <td>
                                <label class="switch" for="{{$station->id}}">
                                        <input type="hidden" name="status[{{$station->id}}]" value="0" />
                                        <input type="checkbox" id="{{$station->id}}"  class="attendance-toggle" name="status[{{$station->id}}]" value="1" @if(isset($check_station[$station->id])&&$check_station[$station->id]==1) checked @endif />
